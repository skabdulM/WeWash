...
function update_service_list(categories) {
    ...
    const pdfDownloadHTML = `
      ...
      >
        ...
        <p>Download complete rate sheet</p>
      </a>`;

    // Clear existing categories and PDF download link
    const service_list_parent = document.querySelector('.service-lists');
    const existingPDFLink = service_list_parent.nextElementSibling;

    if (existingPDFLink && existingPDFLink.tagName === 'A') {
        existingPDFLink.remove(); // Remove the existing PDF link if it exists
    }
    ...
    // Insert the PDF download link after clearing
    service_list_parent.insertAdjacentHTML('afterend', pdfDownloadHTML);
}
